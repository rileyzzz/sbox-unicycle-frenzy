...
namespace UnicycleAI
{
	...
	public class PerceptronBrain : UnicycleBrain
	{
		public NeuronLayer[] Layers { get; set; }
		NeuronLayer Inputs => Layers[0];
		NeuronLayer Outputs => Layers[Layers.Length - 1];

		protected PerceptronBrain()
		{
		}

		public PerceptronBrain( int inputs, int outputs, int layers, int layerSize )
		{
		...
		{
			PerceptronBrain brain = new PerceptronBrain();
		...
		{
			if ( other is not PerceptronBrain b )
				return null;

			PerceptronBrain child = new PerceptronBrain();

			child.Layers = new NeuronLayer[Layers.Length];
			for ( int i = 0; i < child.Layers.Length; i++ )
			{
				NeuronLayer a_layer = Layers[i];
